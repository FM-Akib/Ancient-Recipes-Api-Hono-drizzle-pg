...
export const getAllRecipes = async (c: Context) => {
    try{
        const {region, era, ingredients, limit = 10, page = 1} = c.req.query();

        const conditions = [];
        if (region) conditions.push(eq(recipes.region, region));
        if (era) conditions.push(eq(recipes.era, era));
        if (ingredients) conditions.push(arrayContains(recipes.ingredients, ingredients.split(',')));
...
export const getARecipe = async (c: Context) => {
    try{
...
export const postARecipe =async(c:Context)=>{
    try{
...
export const updateARecipe = async (c:Context)=>{
    try{
        const id = parseInt(c.req.param('id'));
...
export const deleteARecipe = async (c:Context)=>{
    try{
        const id = parseInt(c.req.param('id'));
